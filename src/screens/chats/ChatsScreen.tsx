...
function formatChatTime(ts: string | number | undefined, t: (k: TranslationKeys) => string): string {
  ...
  if (isYesterday(date)) return t('yesterday');   // было захардкожено «Вчора»
...
/**
 * Заглушки ленты историй до появления реального экрана Stories.
 * Имя своей истории берётся из словаря (было захардкожено «Моя Історія»),
 * остальные — демо-имена, они уйдут вместе с заглушкой.
 */
const PLACEHOLDER_STORIES: Array<Omit<StoryItem, 'name'> & { nameKey?: TranslationKeys; name?: string }> = [
  { id: 'own', nameKey: 'your_story', isOwn: true },
  { id: 's1', name: 'Alex M.' },
  { id: 's2', name: 'Sara K.' },
  { id: 's3', name: 'Jordan' },
];

const StoryAvatar: React.FC<{ item: (typeof PLACEHOLDER_STORIES)[number] }> = ({ item }) => {
  ...
  return (
    <TouchableOpacity style={styles.storyItem} activeOpacity={0.7}>
      <View style={[styles.storyRing, { borderColor: theme.primary }]}>
        <Avatar uri={item.avatar} name={label} size={52} />
        {item.isOwn && (
...
const ChatListItem: React.FC<ChatListItemProps> = ({ chat, isOnline, onPress, onLongPress }) => {
  ...
  return (
    <TouchableHighlight
      ...
      // Карточка на фоне экрана — как в Android-версии: surface поверх background.
...
export function ChatsScreen() {
  ...
  if (sandboxEnabled) return <SandboxChatsScreen />;
...
const styles = StyleSheet.create({
  ...
  // Метрики выровнены по Android (ui/chats/ModernChatsUI.kt + ui/theme/WMTokens.kt):
  // карточка с внешним отступом 12, зазором 3 между карточками и внутренними
