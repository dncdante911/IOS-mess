...
const SettingsScreen: React.FC<Props> = ({ navigation }) => {
  ...
  // Hidden dev unlock: 4 taps on version row within 3 seconds
  ...
  const handleVersionTap = useCallback(() => {
    ...
    if (devTapCount.current >= 4) {
      ...
      if (!devUnlocked) {
        setDevUnlocked(true);
        Alert.alert('Режим разработчика', 'Секретный UI разблокирован 🧪');
      }
    }
  }, [devUnlocked]);
  ...
  return (
    ...
    >
      ...
      <Section title={t('settings_appearance')} theme={theme}>
        ...
        {devUnlocked && (
          <TouchableOpacity style={styles.row} activeOpacity={0.7}>
            <View style={styles.rowLeft}>
              <View style={[styles.iconWrap, { backgroundColor: '#40A7E322' }]}>
                <Feather name="zap" size={18} color="#40A7E3" />
              </View>
              <View>
                <Text style={[styles.rowLabel, { color: theme.text }]}>Новый UI (Тест)</Text>
                <Text style={{ fontSize: 11, color: theme.textTertiary, marginTop: 1 }}>
                  Sandbox v4 · только для разработчика
                </Text>
              </View>
            </View>
            <Switch
              ...
              trackColor={{ false: theme.divider, true: '#40A7E366' }}
              thumbColor={sandboxEnabled ? '#40A7E3' : theme.textTertiary}
            />
          </TouchableOpacity>
        )}
      </Section>
      ...
      <Section title={t('settings_about')} theme={theme}>
        ...
        <SettingsRow icon="info" label={t('version')} value="2.0.0" onPress={handleVersionTap} theme={theme} isLast />
      </Section>
