...
//   • E2EE v6 async decryption (Static X3DH, cipherVersion=6)
  ...
  immer((set, get) => ({
    ...
    loadMessages: async (userId) => {
      ...
      try {
        ...
        const decrypted = await Promise.all(msgs.map(tryDecryptMessage));
        set((s) => {
    ...
    loadMoreMessages: async (userId) => {
      ...
      try {
        ...
        const decrypted = await Promise.all(older.map(tryDecryptMessage));
        set((s) => {
    ...
    sendMessage: async (toId, text, replyToId, groupId) => {
      ...
      try {
        // myUserId нужен для self-sync: копия шифруется и для остальных
        // устройств отправителя, иначе своё сообщение не появится ни на
        // Android, ни в Windows, ни в вебе.
        const myUserId = Number(useAuthStore.getState().user?.id ?? 0) || undefined;
        const realMsg = groupId
          ? await chatApi.sendGroupMessage(groupId, text, replyToId)
          : await chatApi.sendMessage(toId, text, replyToId, myUserId);
        set((s) => {
      ...
      socketService.on(SOCKET_EVENT_MESSAGE_REACTION, (data: unknown) => {
        ...
        set((s) => {
          ...
          if (d.action === 'remove') {
            ...
            // count обязателен в MessageReaction: сокет присылает единичное
            // событие «пользователь поставил реакцию», что и есть count = 1.
            if (!exists) arr[idx].reactions = [...reactions, { userId: d.userId, emoji: d.emoji, count: 1 }];
          }
        });
      });
