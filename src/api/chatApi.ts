...
function extractText(raw: Record<string, unknown>): string {
  ...
  // Зашифрованное тело не показываем сырым — расшифровка асинхронная, в chatStore.
  if (ENCRYPTED_CIPHER_VERSIONS.includes(cv)) return '';
...
function extractReplyTo(raw: Record<string, unknown>) {
  ...
  if (reply) {
    const id = str(reply.id ?? reply.message_id ?? '');
    if (!id || id === '0') return {};
    return {
      replyToId: id,
  ...
  const id = str(raw.reply_id ?? raw.message_reply_id ?? '');
  if (!id || id === '0') return {};
  return {
    replyToId: id,
...
export function normaliseMessage(raw: Record<string, unknown>): Message {
  const cv = num(raw.cipher_version);
  // iv/tag/signal_header нужны для расшифровки ЛЮБОЙ зашифрованной версии,
  // а не только текущей: без них chatStore не сможет даже понять, что делать.
  const isEncrypted = ENCRYPTED_CIPHER_VERSIONS.includes(cv);
  ...
  return {
    ...
    iv: isEncrypted ? (raw.iv as string | undefined) : undefined,
 ...
 * POST api/node/chat/chats  { limit, offset }
 * Response shape varies: { data: [...] } or { chats: [...] } or top-level array
 */
export async function getChatsList(page = 1): Promise<Chat[]> {
  ...
  // Server may return chats in different fields — try all known shapes
  let raw: unknown[] = [];
  if (Array.isArray(body)) {
    raw = body;
  } else if (body && typeof body === 'object') {
    const b = body as Record<string, unknown>;
    if (Array.isArray(b.data)) raw = b.data as unknown[];
    else if (Array.isArray(b.chats)) raw = b.chats as unknown[];
    else if (Array.isArray(b.conversations)) raw = b.conversations as unknown[];
    else if (b.data && typeof b.data === 'object' && !Array.isArray(b.data)) {
      const nested = b.data as Record<string, unknown>;
      if (Array.isArray(nested.chats)) raw = nested.chats as unknown[];
      else if (Array.isArray(nested.data)) raw = nested.data as unknown[];
    }
  }

  if (__DEV__) {
    console.log('[getChatsList] count=' + raw.length, 'keys=' + (raw[0] ? Object.keys(raw[0] as object).join(',') : 'empty'));
  }

  return raw.map((item) => normaliseChat(item as Record<string, unknown>));
}
 ...
 */
/**
 * Отправка текста в личный чат.
 *
 * Всегда шифруется по v6. Формат тела запроса — как на Android и Windows:
 * шифротекст кладётся в поле `text` (соглашение Kotlin-клиента), а iv, tag и
 * signal_header едут отдельными полями.
 *
 * ⚠️ FAIL-CLOSED: если зашифровать не удалось (нет бандлов получателя, сеть
 * недоступна, ключи не читаются) — сообщение НЕ отправляется открытым текстом,
 * а бросается ошибка. Молчаливый откат на plaintext в E2EE-мессенджере — это
 * утечка, а не деградация; такую дыру уже закрывали на Android и Windows
 * 14.07.2026, повторять её здесь нельзя.
 *
 * @param myUserId нужен для self-sync: копия шифруется и для остальных
 *                 устройств отправителя, иначе своё же сообщение не будет
 *                 видно на других платформах.
 */
export async function sendMessage(
  ...
  const payload = await getE2EE().encryptForUser(recipientId, text, myUserId ?? 0);

  if (!payload) {
    throw new Error(
      `[chatApi] не удалось зашифровать сообщение для ${recipientId} — отправка отменена`,
    );
  }
  ...
  const res = await nodeApi.post<Record<string, unknown>>(NODE_CHAT_SEND, {
    ...
    cipher_version: payload.cipher_version ?? CIPHER_VERSION_E2EE,
    iv: payload.iv,
    tag: payload.tag,
    signal_header: payload.signalHeader,
...
export async function searchUsers(query: string): Promise<UserSearchResult[]> {
  // GET /api/node/users/search?q=...&limit=30&offset=0  (mirrors Windows/Android)
  const res = await nodeApi.get<Record<string, unknown>>(NODE_SEARCH_USERS, {
