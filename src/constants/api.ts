...
export const NODE_SEARCH_USERS = 'api/node/users/search';
...
/**
 * Актуальный протокол E2EE личных чатов: Static X3DH + SESAME-lite.
 * Единственная версия, которой iOS ШИФРУЕТ. См. crypto/e2ee/e2eeService.ts.
 */
export const CIPHER_VERSION_E2EE = 6;

/**
 * Версии, тело которых зашифровано и НЕ должно показываться как обычный текст.
 *
 *   3 — Double Ratchet (снят с использования; на iOS не поддерживается вовсе —
 *       ключей от той истории здесь нет и быть не может)
 *   4 — групповой GSK
 *   5 — Sender Key v5 (группы) / серверный мастер-ключ (личные)
 *   6 — Static X3DH — текущий
 *
 * Всё, что попало сюда, но не расшифровалось, показывается как «зашифровано»,
 * а не сырым base64.
 */
export const ENCRYPTED_CIPHER_VERSIONS = [3, 4, 5, 6];
